...
void FLuaMachineEditorModule::StartupModule()
{
	...
	StyleSet->SetContentRoot(IPluginManager::Get().FindPlugin("OmegaGameFramework")->GetBaseDir() / "Resources");
	StyleSet->Set("ClassThumbnail.LuaCode", new FSlateImageBrush(StyleSet->RootToContentDir("ico_LuaAsset.png"), FVector2D(128.0f, 128.0f)));
	StyleSet->Set("ClassThumbnail.LuaTableAsset", new FSlateImageBrush(StyleSet->RootToContentDir("ico_LuaTable.png"), FVector2D(256.0f, 256.0f)));
