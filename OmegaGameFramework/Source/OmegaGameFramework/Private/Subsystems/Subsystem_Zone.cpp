...
void UOmegaZoneSubsystem::Tick(float DeltaTime)
{
	...
	case zone_Uninitialzed:
		...
		if(!OGF_Load::LevelStream_IsLoading(GetWorld()))
		{
			...
			l_ZoneInitTime=0.0;
			if(GetMutableDefault<UOmegaSettings>()->bAutoSpawnAtFirstPoint)
			{
	...
	case zone_Initializing:
		...
		if(GetMutableDefault<UOmegaSettings>()->bAutoSpawnAtFirstPoint && zone_Initializing>=GetMutableDefault<UOmegaSettings>()->SpawnAtFirstPointDelay)
		{
			l_LoadState=zone_Idle;
			b_SkipNextFade=true;
...
void UOmegaZoneSubsystem::Local_CompleteTransit()
{
	...
	if(GetWorld() && GetTopLoadedZones() && GetMutableDefault<UOmegaSettings>()->bAutoplayZoneBgm)
	{
...
{
	if(TSubclassOf<AOmegaGameplaySystem> LocalSystem = GetMutableDefault<UOmegaSettings>()->ZoneTransitSystem.LoadSynchronous())
	{
		return LocalSystem;
	}
	return nullptr;
...
void UOmegaZoneSubsystem::Local_OnFinishLoadTask(bool LoadState)
{
	if (LoadState)
	{
        ...
        {
			if(GetMutableDefault<UOmegaSettings>()->bDynamicCameraViewTargetOnTransit)
			{
...
{
	if(ULevelSequence* seq = GetMutableDefault<UOmegaSettings>()->Sequence_ZoneTransit.LoadSynchronous())
	{
		return seq;
	}
	return nullptr;
