	...
	for (int32 i = StartRow; i < Lines.Num(); i++)
	{
		...
		if (bHasHeader && Headers.Num() > 0)
		{
			...
			for (int32 j = 0; j < Fields.Num(); j++)
			{
				...
				// Check if the key contains a dot - if so, create nested structure
				if (Key.Contains(TEXT(".")))
				{
					SetNestedField(RowTable, Key, FLuaValue(Fields[j]));
				}
				else
				{
...
void UOmegaLuaParser_CSV::SetNestedField(FLuaValue& Table, const FString& Path, const FLuaValue& Value)
{
	// Split path by dots
	...
	// If only one part, just set it directly
	if (PathParts.Num() == 1)
	{
		Table.SetField(PathParts[0], Value);
		return;
	}
	...
	for (int32 i = 0; i < PathParts.Num() - 1; i++)
	{
		...
		// Check if this part is a number (for array-style access)
		bool bIsNumeric = Part.IsNumeric();
		FLuaValue NextTable;
		...
		if (bIsNumeric)
		{
			...
			NextTable = CurrentTable.GetFieldByIndex(Index);
		}
		else
		{
			// Access by key
			NextTable = CurrentTable.GetField(Part);
		}
		...
		if (NextTable.IsNil())
		{
			...
			{
				int32 Index = FCString::Atoi(*Part);
				CurrentTable.SetFieldByIndex(Index, NextTable);
			}
			else
			{
				CurrentTable.SetField(Part, NextTable);
			}
		}
	...
	// Set the final value
	FString LastPart = PathParts.Last();
	
	if (LastPart.IsNumeric())
	{
		// Set by index
		int32 Index = FCString::Atoi(*LastPart);
		CurrentTable.SetFieldByIndex(Index, Value);
	}
	else
	{
		// Set by key
		CurrentTable.SetField(LastPart, Value);
	}
}
