...
Input focus is handled automatically: the topmost widget implementing the **Widget Input interface** receives routed input.
...
Control it with the **🎥Dynamic Camera** function library nodes:

| Node | Description |
|---|---|
| **Dynamic Camera - Set Active** | Turn the dynamic camera system on/off for a player |
| **Dynamic Camera - Is Active?** | Query whether it's running |
| **Dynamic Camera - Get MASTER / Get SOURCE** | Get the current master or source camera |
| **Dynamic Camera - Set OVERRIDE** | Give a camera full control (optionally snapping to it) |
