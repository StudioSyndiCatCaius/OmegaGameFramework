...
| Node | Description |
|---|---|
| **Get Children Widgets** | Returns a widget's children, optionally recursive, filtered by class. |
...
| **Try Get Data Widget From Slot** | Returns the `UDataWidget` registered in a tagged slot. **Outcome** exec fires only if valid. |
| **Data Widget — Get First from Source / Get All from Sources** | Find active data widgets displaying a given source object (or objects). |
