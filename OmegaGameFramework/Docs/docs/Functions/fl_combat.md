...
Covers: `F_Combatant`, `F_Equipment`, `F_Inventory`, `F_ScriptedEffects`, `F_CombatantFilter`
...
| Node | Description |
|---|---|
| **🗡️Equipment - Filter Equippables** | From a list of items, returns only those valid for a given slot. |
...
| Node | Description |
|---|---|
| **Filter Combatants by Interface** | Passes an array of combatants through a skill-interface filter. Returns only those that pass. |
| **Select First Skill That Can Target** | From a list of skill assets, returns the first one that can validly target a given combatant. Outputs an **Outcome** exec. |

> For reusable filters, create **Combatant Filter Script** Blueprints and group them in a **Combatant Filter Collection** asset (used by skill configs for target filtering).
