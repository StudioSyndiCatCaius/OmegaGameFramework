...
|---|---|
| **Calendar Asset** | The `UOmegaCalendar` data asset that defines the structure (how many months, days per week, period names, etc.). |
...
| **Get Period Asset** | Returns the `UOmegaDayPeriod` asset for the current period (has the name, icon, etc.). |
| **Get Day Number** | Returns the current day index. |
| **Get Day Asset** | Returns the `UOmegaDay` asset for the current day. |
| **Get Month Number** | Returns the current month index. |
| **Get Month Asset** | Returns the `UOmegaMonth` asset for the current month. |
